...
class ExifLatLonApp:
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.root.title("Exif lat/lon exporter → CSV + GeoJSON")
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(frame, text="Image folder:").grid(row=0, column=0, sticky="w")
        ttk.Entry(frame, textvariable=self.input_folder).grid(
            row=1,
            column=0,
            sticky="ew",
            padx=(0, 8),
        ...
        ttk.Button(frame, text="Browse folder", command=self.pick_folder).grid(
            row=1,
            column=1,
            sticky="ew",
            pady=(4, 10),
        ...
        ttk.Label(frame, text="CSV output file:").grid(row=2, column=0, sticky="w")
        ttk.Entry(frame, textvariable=self.output_file).grid(
            row=3,
            column=0,
            sticky="ew",
            padx=(0, 8),
        ...
        ttk.Button(frame, text="Browse file", command=self.pick_output).grid(
            row=3,
            column=1,
            sticky="ew",
            pady=(4, 10),
        ...
        ttk.Combobox(
            options,
            textvariable=self.extractor,
            values=["auto", "exiftool", "pillow"],
            width=12,
            state="readonly",
        ...
        info = (
            "Auto uses ExifTool if it is available on your system. "
            "This is the recommended path for RAW, HEIF/HEIC, and JPG files. "
            "If ExifTool is not available, the program falls back to Pillow/pillow-heif."
        )
        ttk.Label(
            frame,
            text=info,
        ...
        self.run_button = ttk.Button(frame, text="Run export", command=self.run)
    ...
    def _run_worker(self) -> None:
        try:
            self.root.after(0, lambda: self.log("Starting scan..."))
            ...
            csv_path, geojson_path, summary_path, no_gps_path = write_outputs(
                result,
            ...
            lines = [
                ...
                f"Supported files scanned: {result.total_supported_files}",
                f"Files with GPS: {result.gps_count}",
                f"Files without GPS: {result.no_gps_count}",
            ...
            if no_gps_path:
                lines.append(f"Files without GPS list: {no_gps_path}")
